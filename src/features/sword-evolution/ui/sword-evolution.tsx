 ...
 * 검 진화 컴포넌트
 */
export function SwordEvolution({
  sword,
  onEvolve,
  isLoading,
}: SwordEvolutionProps) {
  const [evolving, setEvolving] = useState(false);
  const stageInfo = getStageInfo(sword.stage);
  const maxStage = 5;

  // 진화 처리 함수
  const handleEvolve = () => {
    // 진화 가능 여부 확인
    if (sword.stage >= maxStage || evolving || isLoading) return;
  ...
  return (
    <div className="bg-white bg-opacity-90 rounded-lg shadow-lg p-5 w-full max-w-md">
      ...
      <button
        ...
        disabled={sword.stage >= maxStage || evolving || isLoading}
      >
        {sword.stage >= maxStage
          ? "최대 단계 도달"
          : evolving
